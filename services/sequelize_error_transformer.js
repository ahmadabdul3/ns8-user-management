...
function handleErrorsObject(errors) {
  ...
  error.message = getErrorFromType(firstError);
...
function validationError(error) {
  ...
  switch (validatorKey) {
    ...
    case 'isEmail':
      return 'The provided email has an incorrect format';
    case 'phoneFormat':
      return `Phone numbers must have the following format: ###-###-####`;
    case 'notEmpty':
      return `The ${path} field can't be empty`;
    default:
      return FALLBACK_ERROR_MESSAGE;
  }
}
